...
- **Brand accent is the 2026 logo's burgundy family (July 2026 rebrand; was green).** Tokens are `--color-accent` / `-dark` / `-bright` / `-glow`; utilities are `text-accent`, `bg-accent`, etc. (the old `*-green` utilities no longer exist). Accent is theme-tuned: lifted wine `#c9515c` on dark, the logo's true deep maroon `#7d2730`/`#63171d` on light. Glows everywhere derive from the token via `color-mix(... var(--color-accent) ...)`, so future palette changes are a globals.css edit. Only satori OG images, email templates, and favicon.svg carry burgundy hex literals (they cannot use CSS vars). `--color-logomark` colors the LogoMark tree: cream on dark, maroon on light.
...
- **July 24, 2026: newsletter retired.** Despite the July 21 hardening, bot signups kept trickling in daily (rotating IPs across serverless instances sidestep per-instance limits), so Will retired the newsletter entirely. Removed: the footer signup, both journal signup blocks (replaced with RSS pointers), and `NewsletterSignup.tsx` (deleted; in git history with the old hardened API if it ever returns, in which case rebuild it with double opt-in). `/api/subscribe` is now a deliberate no-op stub answering `{ok:true}` so lingering bots get a convincing success while nothing sends, notifies, or stores, ever. Privacy policy updated to match (newsletter references removed, retirement noted, Last updated bumped to July 24). RSS (`/feed.xml`) is the sole subscription mechanism. Separately: Will's DMARC report flood traced to `rua=mailto:info@wildtechdev.com` in the `_dmarc.wildtechdev.com` TXT record on Namecheap DNS; fix is his (drop the rua tag), not a code change.
- **July 22, 2026: full burgundy rebrand plus the 2026 logo.** Will delivered the new oak-circuit logo (`public/WildTech_logo_2026.svg`, maroon `#63171d` and black with Charter wordmark text) and chose a full accent rebrand from green to the logo's burgundy family. Shipped in two pushes: (1) `LogoMark.tsx` (script-generated from the SVG, art only, currentColor + knockouts in the page bg token) in the navbar and footer with the `--color-logomark` token; (2) the rebrand: green tokens renamed to the semantic `--color-accent` family with theme-tuned values (dark: wine `#c9515c`/`#8f3540`/`#e07a84`; light: maroon `#7d2730`/`#63171d`/`#96343f`), all `*-green` utility classes renamed to `*-accent` across 35 files, and every one of the ~108 hardcoded green literals converted to `color-mix(in srgb, var(--color-accent) N%, transparent)` token references (underscore syntax inside Tailwind arbitrary values), so glows/shadows/gradients now follow the theme automatically. Hex swaps where CSS vars are impossible: the 7 satori OG image files, both email templates (wine on the dark header band, deep maroon links on white bodies), the print stylesheet link color, and favicon.svg (maroon W tile, interim). Cursor label pill flipped to cream-on-burgundy text (dark-on-accent failed contrast once the accent went dark). Verified: typecheck, eslint, full build clean; visual pass on home and journal in both themes (green haze fully gone, warm wine/blush glows). Left alone on purpose: per-product card hover tint keys in `page.tsx`/`HomeProductCard` (product personalities, not brand), journal prose mentioning green as a word. STILL TO DO (needs Will/Firefly): outline the Charter wordmark text in the master SVG before using the full lockup as an image anywhere (Illustrator Type > Create Outlines), and a simplified tree glyph for favicon/app icons (the full mark is unreadable at 16px; app icon PNGs in public/ still show the old green branding). A bot flooded `/api/subscribe` with dotted-Gmail variants of one address (`a.nneh.ende.rson.6.1.1@gmail.com` style; Gmail ignores dots and plus tags, so every variant delivers to the same victim inbox). Each hit sent a welcome email to the victim and a notification email to info@, flooding both and risking spam complaints against the domain. Hardened the route with four layers, all documented in `src/app/api/subscribe/route.ts`: (1) best-effort per-IP rate limit (3 per 10 min, mirrors the contact API), (2) Gmail dot/plus normalization with a per-instance dedupe set (repeat and variant signups answer success but send nothing; failed sends release the slot so retries work), (3) a SUPPRESSED set for weaponized addresses (currently the one victim, compared normalized; add future ones there), (4) cross-site Origin rejection (honeypot-style silent success; localhost allowed for dev). Verified by simulated attack against the dev server: variants collapse to one processed signup, suppressed/honeypot/cross-origin all no-op with zero log lines, 4th request from one IP gets 429, legit signups still process. NOTE for Will: the in-memory limits are per serverless instance; for a hard guarantee enable a Vercel Firewall rate-limit rule on `/api/subscribe`, and check the Resend dashboard for quota burn from the attack.
...
- ~~Set `RESEND_AUDIENCE_ID` env var~~ OBSOLETE: the newsletter was retired July 24, 2026 (see Recent work).
